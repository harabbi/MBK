...
cnt ="1"
["new", "update"].each() { |t|
  unless $con.execute("SELECT COUNT(*) FROM #{export_db}.Products_Joined WHERE mbk_import_#{t}=1").first.first.to_i == 0
  ...
  $con.execute("SELECT * FROM #{export_db}.Products_Joined where mbk_import_#{t}=1").each() { |r|  
    ...
    i=0; r.each() { |rr|  
      ...
      rh[cols[i].to_s] = rr.to_s
      i=i+1 
    }
    pcode = rh["productcode"].to_s
#puts "rh--------- #{rh["productname"]}"
    $con.execute("SELECT * FROM mbk_site_export_#{Time.now.strftime("%Y%m%d")}.Products_Joined where productcode='#{pcode}' limit 1").each() { |p| 
      5.times() { |ii| p.pop }
      ii=0; p.each() { |pp| ph[pcols[ii].to_s] = pp; ii=ii+1 }
    }
#puts "ph--------- #{ph["productname"]}"
    if ph.size == 0 and t == "new"
      ...
      insert_statement << cols.join(",").gsub(/categoryids/, "CategoryIDs")
      insert_statement << ") VALUES ("
      cols.each() { |c|
        begin
          if coltype[c] == "text" or coltype[c].split("(").first.strip == "varchar" or coltype[c] == "datetime"
            insert_statement << "'#{Mysql.escape_string(rh[c])}',"
          else
      ...
      begin
        ...
        $con.execute("update mbk_site_export_#{Time.now.strftime("%Y%m%d")}.Products_Joined set mbk_ready_to_import=1 where productcode='#{rh["productcode"]}'")
      rescue
      ...
      cols.each() { |c|
        unless rh[c].blank? or ["categoryids","categoryid"].include?(c)
          unless rh[c].to_s == ph[c].to_s
            mbklogdebug(__FILE__, "#{rh["productcode"]} -- column #{c} is different: old=#{ph[c]} new=#{rh[c]}")
            begin
              if coltype[c] == "text" or coltype[c].split("(").first.strip == "varchar" or coltype[c] == "datetime"
          #      rh[c] = rh[c].to_s.gsub("\\","")
                $con.execute("update mbk_site_export_#{Time.now.strftime("%Y%m%d")}.Products_Joined set #{c}='#{Mysql.escape_string(rh[c])}',   mbk_ready_to_import=1 where productcode='#{rh["productcode"]}'") unless ph[c].to_s == rh[c].to_s
              else
